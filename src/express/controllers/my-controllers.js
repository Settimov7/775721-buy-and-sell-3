...
exports.getMyComments = async (req, res) => {
  ...
  const commentRequests = userOffersIds.map((id) => request.get({url: `http://localhost:3000/api/offers/${id}/comments`, json: true}));
  const commentResponses = await Promise.all(commentRequests);
  const userComments = commentResponses.map(({ statusCode, body }) => statusCode === HttpStatusCode.OK ? body: []);
